...
export default function NotificationsPage() {
  ...
  const [notifications, setNotifications] = useState([
    {
      ...
      message:
        "Your career guidance appointment request on Tuesday, April 15, 2025 at 10:30 AM has been accepted by Dr. Ahmed Hassan (SCAD Officer).",
  ...
  useEffect(() => {
    try {
      ...
      if (savedNotifications && savedNotifications.length > 0) {
        // Process localStorage notifications to match the structure expected by the UI
        const processedNotifications = savedNotifications.map(
          (notification) => {
            // Convert the icon to JSX based on type
            let icon;
            switch (notification.type) {
              case "appointment_status":
                icon = notification.message.includes("accepted") ? (
                  <CheckCircle className="h-5 w-5" />
                ) : (
                  <XCircle className="h-5 w-5" />
                );
                break;
              case "internship_cycle":
                icon = <Calendar className="h-5 w-5" />;
                break;
            ...
            return {
              ...
              link: notification.link || "/dashboard/pro-student/appointments",
            };
          }
        );

        // Merge with existing hardcoded notifications, avoiding duplicates by ID
    ...
    setNotifications(
      notifications.map((notification) => {
        if (notification.id === notificationId) {
          // Update localStorage if this is a saved notification
          try {
            const savedNotifications = JSON.parse(
              localStorage.getItem("notifications") || "[]"
            );
            const updatedSavedNotifications = savedNotifications.map((n) =>
              n.id === notificationId ? { ...n, read: true } : n
            );
            localStorage.setItem(
              "notifications",
              JSON.stringify(updatedSavedNotifications)
            );
          } catch (error) {
  ...
  const deleteNotification = (notificationId) => {
    ...
    try {
      ...
      const updatedSavedNotifications = savedNotifications.filter(
