...
function CallPageContent() {
  ...
  useEffect(() => {
    ...
    // Simulate advisor leaving call after random time (2-5 minutes) for demo
    const advisorLeaveTimer = setTimeout(() => {
      if (Math.random() > 0.5) {
        // 50% chance
        setAdvisorLeft(true);
        showNotification(
          "Advisor left the call",
          `${callDetails?.advisor || "Advisor"} has left the meeting`
        );
      }
    ...
    return () => {
      clearInterval(timer);
      clearTimeout(advisorLeaveTimer);
    };
  }, [callId, callDetails?.advisor]);
  ...
  return (
    <div className="h-screen bg-black flex flex-col">
      ...
      {/* Advisor left notification */}
      {advisorLeft && isCallActive && (
        <div className="fixed top-0 inset-x-0 z-40 bg-yellow-500/10 border-b border-yellow-500/30 p-3 flex items-center justify-center">
          ...
          <p className="text-yellow-400 text-sm">
            {callDetails.advisor} has left the call. You can leave the call or
      ...
      <div className="flex-1 flex flex-col">
              ...
              <>
                {advisorLeft ? (
                  <div className="text-center">
                    ...
                    <h3 className="text-white text-lg font-medium">
                      {callDetails.advisor} has left the call
                    </h3>
                  </div>
                ) : (
                  <div className="text-center">
        ...
        <div className="fixed right-0 top-0 h-full w-80 bg-black border-l border-gray-800 p-4 flex flex-col">
          ...
          <div className="py-4 border-b border-gray-800">
            <h3 className="text-sm font-medium text-white mb-2">Advisor</h3>
            <div className="flex items-center">
              <div className="h-8 w-8 rounded-full bg-[#FF6F1B]/20 flex items-center justify-center mr-2">
                <span className="text-[#FF6F1B] text-xs font-bold">
                  {callDetails.advisor?.charAt(0) || "A"}
                </span>
              </div>
              <div>
                <p className="text-sm text-white">{callDetails.advisor}</p>
                <div className="flex items-center text-xs">
                  <span
                    className={`w-2 h-2 rounded-full ${
                      !advisorLeft ? "bg-green-500" : "bg-gray-500"
                    } mr-1`}
                  ></span>
                  <span
                    className={`${
                      !advisorLeft ? "text-green-400" : "text-gray-400"
                    }`}
                  >
                    {!advisorLeft ? "Online" : "Offline"}
                  </span>
                </div>
              </div>
            </div>
