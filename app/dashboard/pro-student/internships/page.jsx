...
export default function InternshipsPage() {
  ...
  // Add state for PRO badge modal
  const [isProBadgeModalOpen, setIsProBadgeModalOpen] = useState(false);
  ...
  // Add state for handling appointment check
  const [hasApprovedAppointment, setHasApprovedAppointment] = useState(false);

  // Sample appointments data (this would come from an API in a real application)
  const appointments = [
    {
      id: 1,
      date: "2025-04-10",
      ...
      notes: "Please prepare your resume and portfolio for discussion",
  ...
  // Check if student has completed at least one 3-month internship
  const hasCompletedThreeMonthInternship = () => {
  ...
  // Get total months of internship experience
  const getTotalInternshipMonths = () => {
    let total = 0;
    internships.forEach((internship) => {
      if (internship.isCompleted) {
        if (internship.duration.includes("3 months")) total += 3;
        else if (internship.duration.includes("4 months")) total += 4;
        else if (internship.duration.includes("6 months")) total += 6;
      }
    });
    return total;
  };

  // Count number of completed internships
  const getCompletedInternshipsCount = () => {
    return internships.filter((internship) => internship.isCompleted).length;
  };

  // Function to check if user has an approved appointment
  const checkHasApprovedAppointment = () => {
  ...
  useEffect(() => {
    setHasApprovedAppointment(checkHasApprovedAppointment());
  }, []);

  // New handler for video button click - always open the video modal
  ...
  return (
    <div className="space-y-8">
      {/* Test Controls - For development only, remove in production */}
      <div className="bg-gray-800/30 border border-gray-700 p-2 rounded-md flex items-center justify-end mb-4">
        <div className="text-xs text-gray-400 font-mono flex items-center">
          <span className="mr-2">[DEV MODE]</span>
          <span className="mr-2">Has Approved Appointment:</span>
          <Button
            variant="ghost"
            className="p-1 h-auto"
            onClick={() => setHasApprovedAppointment(!hasApprovedAppointment)}
          >
            {hasApprovedAppointment ? (
              <>
                <ToggleRight className="h-5 w-5 text-green-400" />{" "}
                <span className="ml-1 text-green-400">True</span>
              </>
            ) : (
              <>
                <ToggleLeft className="h-5 w-5 text-gray-400" />{" "}
                <span className="ml-1 text-gray-400">False</span>
              </>
            )}
          </Button>
        </div>
      ...
      <div className="flex flex-col md:flex-row justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white flex items-center">
            ...
            {hasCompletedThreeMonthInternship() && (
              <Badge
                className="ml-3 bg-gradient-to-r from-[#EC1024] to-[#FF6F1B] text-white border-0 px-3 py-1 text-xs cursor-pointer animate-pulse hover:animate-none"
                onClick={() => setIsProBadgeModalOpen(true)}
              >
                PRO
              </Badge>
            )}
        ...
        <div className="flex space-x-2">
          {hasCompletedThreeMonthInternship() && (
            <Button
              ...
              onClick={() => setIsProBadgeModalOpen(true)}
            >
              <Badge className="bg-gradient-to-r from-[#EC1024] to-[#FF6F1B] text-white border-0 mr-2">
                PRO
              </Badge>
              My Achievements
            </Button>
          ...
          >
            ...
            {!hasApprovedAppointment && <Lock className="h-3 w-3 ml-2" />}
          </Button>
        </div>
      </div>
      ...
      {/* PRO Badge Achievement Modal */}
      <Dialog open={isProBadgeModalOpen} onOpenChange={setIsProBadgeModalOpen}>
        <DialogContent className="bg-black border-gray-800 text-white max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-gray-400 text-center">
              Congratulations on completing your internship requirements!
            </DialogDescription>
          </DialogHeader>

          <div className="py-4">
            <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4 bg-gradient-to-r from-[#EC1024] to-[#FF6F1B]">
              <Badge className="text-2xl bg-transparent text-white border-0">
                PRO
              </Badge>
            </div>
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-gray-400 mt-1">
                You've completed your required internship experience
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div className="bg-gray-900/30 p-4 rounded-lg">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-white">
                    {getTotalInternshipMonths()} months
                  </span>
                </div>
              </div>
              ...
              <div className="bg-gray-900/30 p-4 rounded-lg">
                <div className="flex justify-between">
                  <span className="text-gray-400">Internships Completed:</span>
                  <span className="font-bold text-white">
                    {getCompletedInternshipsCount()}
                  </span>
                </div>
              </div>
              ...
              <div className="bg-gray-900/30 p-4 rounded-lg">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-[#FF6F1B]">PRO Student</span>
                </div>
              </div>
            </div>

            <div className="mt-6 bg-[#FF6F1B]/10 border border-[#FF6F1B]/30 rounded-lg p-4 text-center">
              <p className="text-gray-300 text-sm">
                Your PRO status gives you access to exclusive opportunities,
                premium career services, and connects you with our top industry
                partners.
              </p>
            </div>
          </div>
          ...
          <DialogFooter className="flex justify-center">
            <Button
              onClick={() => setIsProBadgeModalOpen(false)}
              className="bg-gradient-to-r from-[#EC1024] to-[#FF6F1B] text-white"
            >
              Continue as PRO Student
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isVideoModalOpen} onOpenChange={setIsVideoModalOpen}>
        <DialogContent className="bg-black border-gray-800 text-white max-w-3xl">
          ...
          {hasApprovedAppointment ? (
                ...
                <div className="w-full h-full flex items-center justify-center relative">
                  ...
                  <div className="z-10 text-center">
                    ...
                    <p className="mt-4 text-sm text-gray-400 max-w-md mx-auto">
                      This video explains the specific internship requirements
                      for Computer Science, Engineering, Business and other
                      majors at GUC.
                    </p>
                  </div>
                </div>
            ...
            <>
              <div className="aspect-video w-full bg-gray-900/70 rounded-md overflow-hidden relative">
                <div className="absolute inset-0 bg-[url('/assets/video-placeholder.jpg')] bg-cover bg-center opacity-20"></div>
                <div className="absolute inset-0 bg-black/70 backdrop-blur-sm"></div>
                <div className="absolute inset-0 flex items-center justify-center flex-col">
                  <div className="h-20 w-20 rounded-full bg-yellow-500/20 flex items-center justify-center mb-4">
                  ...
                  <p className="text-gray-400 mt-2 max-w-md text-center">
                    This video is only available to students with approved
                    appointments
                  </p>
                </div>
              </div>
              ...
              <div className="bg-gray-900/30 p-4 rounded-lg mt-4">
                ...
                <p className="text-gray-300 text-sm mb-4">
                  Meeting with an advisor helps ensure you choose internships
                  that align with your academic requirements and career goals.
                </p>
                <div className="bg-[#FF6F1B]/10 border border-[#FF6F1B]/30 rounded-lg p-3">
                  <p className="text-white text-sm">
                    <Calendar className="h-4 w-4 inline mr-2 text-[#FF6F1B]" />
                    Schedule an appointment with an advisor to unlock this
                    content and get personalized guidance.
                  </p>
                </div>
              </div>
              ...
              <DialogFooter className="flex justify-between mt-4">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    // This would navigate to appointments page in a real app
